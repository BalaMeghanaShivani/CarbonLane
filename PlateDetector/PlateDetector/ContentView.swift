        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            DispatchQueue.main.async {
                if self?.yoloDetector.isReady == true {
                    self?.statusMessage = "Model ready — point camera at a car"
                    self?.cameraManager.start()
                } else {
                    self?.statusMessage = "⚠️ Failed to load model. Check that yolo26s_xnnpack.pte is in the app bundle."
                }
            }
        }
            ...
            if viewModel.cameraManager.permissionGranted {
                ...
                VStack(spacing: 16) {
                    ...
                    Text("Camera access required")
                        .font(.title2)
                    Text("Go to Settings → PlateDetector → Camera")
                        .font(.caption)
